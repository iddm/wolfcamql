...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset, qboolean keepLightmap, qboolean userSet) {
	...
	for (sh = hashTable[hash]; sh; sh = sh->next) {
		if (Q_stricmp(sh->name, strippedName) == 0) {
			if (sh != sh2) {
				if (userSet) {
					sh->userRemappedShader = sh2;
					sh->remappedShaderKeepLightmap = keepLightmap;
				} else {
					sh->gameRemappedShader = sh2;
					sh->remappedShaderKeepLightmap = keepLightmap;
				}
			} else {
				if (userSet) {
					sh->userRemappedShader = NULL;
					sh->remappedShaderKeepLightmap = qfalse;
				} else {
					sh->gameRemappedShader = NULL;
					sh->remappedShaderKeepLightmap = qfalse;
				}
			}
		}
...
void R_ClearRemappedShader (const char *shaderName)
{
	...
	if (sh == NULL || sh == tr.defaultShader) {
		ri.Printf(PRINT_WARNING, "WARNING: %s: shader %s not found\n", __FUNCTION__, shaderName);
		return;
	}
...
static void ParseSkyParms( char **text ) {
	...
	r_cloudHeightOrig = ri.Cvar_Get("r_cloudHeightOrig", va("%d", (int)shader.sky.cloudHeight), CVAR_ROM);
			...
			case RC_DRAW_SURFS:
				...
				for( i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs; i++, drawSurf++ ) {
					...
					if( sortedIndex >= newShader ) {
						sortedIndex++;
						drawSurf->sort = ((uint64_t)sortedIndex << QSORT_SHADERNUM_SHIFT) | (uint64_t)entityNum | (uint64_t)( fogNum << QSORT_FOGNUM_SHIFT ) | (uint64_t)( (int)pshadowMap << QSORT_PSHADOW_SHIFT) | (uint64_t)dlightMap;
					}
				}
...
static void ScanAndLoadShaderFiles( void )
{
	...
	for ( i = 0; i < numShaderFiles + numOverrideShaderFiles; i++ )
	{
		...
		// look for a .mtr file first
		{
			...
			if (i < numOverrideShaderFiles) {
				Com_sprintf( filename, sizeof( filename ), "shaderoverride/%s", overrideShaderFiles[i] );
			} else {
				Com_sprintf( filename, sizeof( filename ), "scripts/%s", shaderFiles[i - numOverrideShaderFiles] );
			}
	...
	s_shaderText = ri.Hunk_Alloc( sum + numShaderFiles*2 + numOverrideShaderFiles*2, h_low );
	s_shaderText[ 0 ] = '\0';
...
void R_CreateSingleShader (void)
{
	...
	if (*r_singleShaderName->string) {
		if (r_vertexLight->integer) {
			tr.singleShader = R_FindShader(r_singleShaderName->string, LIGHTMAP_BY_VERTEX, qtrue);
		} else {
			qboolean hasLightmap = qfalse;
			shaderStage_t *pStage;

			tr.singleShader = R_FindShader(r_singleShaderName->string, LIGHTMAP_NONE, qtrue);

			// check if it has lightmap
			for (stage = 0;  stage < MAX_SHADER_STAGES;  stage++) {
				pStage = tr.singleShader->stages[stage];
			...
			if (!hasLightmap) {
				...
				tr.singleShader->stages[0]->bundle[0].image[0] = R_FindImageFile(r_singleShaderName->string, IMGTYPE_COLORALPHA, IMGFLAG_MIPMAP | IMGFLAG_PICMIP);  // use GL_REPEAT
...
void RE_ReplaceShaderImage (qhandle_t h, const ubyte *data, int width, int height)
{
	...
	//FIXME if width and height unchanged?
	//R_UpdateSubImage(image, (byte *)data, 0, 0, width, height, GL_RGBA8);

	//FIXME discarding const: (byte *)
	R_CreateImage2(image->imgName, (byte *)data, width, height, GL_RGBA8, 0, image->type, image->flags, 0, image);
...
qhandle_t RE_RegisterShaderFromData (const char *name, ubyte *data, int width, int height, qboolean mipmap, qboolean allowPicmip, int wrapClampMode, int lightmapIndex)
{
	...
	//FIXME discarding const :  (ubyte *)data
	//image = R_CreateImage(name, (ubyte *)data, width, height, mipmap, allowPicmip, wrapClampMode);
	image = R_CreateImage(name, (ubyte *)data, width, height, IMGTYPE_COLORALPHA, IMGFLAG_CLAMPTOEDGE, 0);
