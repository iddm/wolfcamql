...
void FS_FOpenSysFileRead (const char *filename, fileHandle_t *file)
{
	...
	*file = FS_HandleForFile();
	...
	Q_strncpyz(fsh[*file].name, ospath, sizeof(fsh[*file].name));
	fsh[*file].zipFile = qfalse;
	if (fs_debug->integer) {
		Com_Printf("FS_FOpenSysFileRead: %s\n", ospath);
	}
}
...
char **FS_ListFilteredFiles( const char *path, const char *extension, char *filter, int *numfiles, qboolean allowNonPureFilesOnDisk ) {
	...
	if (!Q_stricmpn(path, "ql:", strlen("ql:"))) {
		int numSysFiles;
		char **sysFiles;
		char *name;
		char ospath[MAX_OSPATH];
...
void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	//Com_Printf("^3FS_AddGameDirectory('%s', '%s')\n", path, dir);
...
#if 0
	if (*fs_homepath->string  &&  *fs_gamedirvar->string) {
		fs_shaderSearchPaths = Z_Malloc(sizeof(searchpath_t));
		fs_shaderSearchPaths->dir = Z_Malloc(sizeof(*fs_shaderSearchPaths->dir));
		Q_strncpyz(fs_shaderSearchPaths->dir->path, va("%s/%s", fs_homepath->string, fs_gamedirvar->string), sizeof(fs_shaderSearchPaths->dir->path));
		Q_strncpyz(fs_shaderSearchPaths->dir->gamedir, "shaderoverride", sizeof(fs_shaderSearchPaths->dir->gamedir));
		//fs_shaderSearchPaths->next = fs_searchpaths;
	}
#endif
...
void FS_InitFilesystem( void ) {
	...
	if ( FS_ReadFile( "default.cfg", NULL ) <= 0 ) {
		...
		//FIXME check for quake live paks
		//Com_Printf("Couldn't load default.cfg\n");
		//Cvar_Set("com_errorMessage", "quake live paks not found");
		com_notInstalled = qtrue;
...
void FS_Restart( int checksumFeed ) {
	...
	if ( Q_stricmp( FS_GetCurrentGameDir(), lastGameDir ) ) {
		...
		if ( !Com_SafeMode() ) {
			...
			//FIXME wolfcam hack to override default.cfg
